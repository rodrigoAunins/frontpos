...
// IMPORTA TU MODAL PARA CANCELAR
...
export default function Navbar({ currentUser, currentSession, setCurrentSession, onLogout }) {
  ...
  // NUEVO: estado para mostrar/ocultar el modal
  ...
  return (
    <>
          ...
          <ul className="navbar-nav mr-auto">
            ...
            {currentUser?.role === 'cashier' && (
              <>
                ...
                {/* NUEVO: Botón que abre el modal de cancelar venta */}
                <li className="nav-item">
                  <button
                    className="btn btn-sm btn-danger ml-2"
                    onClick={() => setShowCancelSaleModal(true)}
                  >
                    <i className="fa-solid fa-ban"></i> Cancelar Venta
                  </button>
                </li>
              </>
            )}
      ...
      {/* Mostrar el Modal de Cancelar Venta si showCancelSaleModal = true */}
      {showCancelSaleModal && (
        <CancelSaleModal
